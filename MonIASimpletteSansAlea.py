...
class MonIASimpletteSansAlea(JoueurMarrakech):
    ...
    def _eval(self, modele, d):
        """ version parano : OK pour minimax """
        self.stat_feuilles+=1
        points = modele.points()
        if len(modele.tapis[-1]) == 0:
            # endgame : le dernier joueur n'a plus de tapis
            res=float('Inf') # plus l'infini existe en Python même si ce n'est pas un entier on peut le comparer avec eux
